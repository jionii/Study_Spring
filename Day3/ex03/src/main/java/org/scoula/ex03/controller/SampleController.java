...
public class SampleController {
    ...
    public String sample() {
        ...
        return "sample"; // /WEB-INF/views/sample.jsp ==> 없는 경우 404가 발생함
    }

    @GetMapping("/basicOnlyGet")
    public void basicOnlyGet() {
        // void인 경우에 호출한 주소를 사용해서 /WEB-INF/views/요청주소.jsp
        // 요청주소와 views 밑에 파일 이름이 같다면 String으로 return 안 해도 됨
    ...
    public void basic() {
        log.info("/basic ================== ");
        // 스프링은 컨트롤러로 오면 그 다음은 무조건 jsp파일을 호출하는 흐름
...
//        int age = Integer.parseInt(request.getParameter("age"));
    ...
    @GetMapping("/ex011")
    // 전달되는 데이터가 많은 경우 가방 역할의 변수에 넣어달라고 요청하면 됨
    // dto에 있는 파라미터와 동일한 set 메서드를 불러서 가방에 넣어줌
    // 컨트롤러 함수 호출시 입력 파라미터로 넣은 dto는 자동으로 view까지 전달됨
    // Model의 속성으로 추가가 됨

    // *주의*
    // 변수명을 첫 글자만 클래스 이름과 다르게 소문자로 만들어야함
