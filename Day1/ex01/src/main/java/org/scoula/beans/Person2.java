...
public class Person2 {
    ...
    //    Person2 먼저 만들고 나중에 Parrot을 끼는 방식, 불호
    // 2. 생성자에 표시 --> 권장! 미리 만들어서 객체 생성할 때 주입!
    // 3. set 메서드에 표시 --> 비권장! Parrot을 나중에 끼고 싶을 때 사용(가끔 사용)

    private String name;

    // 싱글톤 중에서 Parrot2의 클래스로 만들어진 것 있으면 자동으로 주입해줘
//    @Autowired
    // 생성자로 주입한 것은 final을 써서 처음에 주입한 parrot을 계속 사용할 수 있는 안정성을 확보
    // 생성자 주입의 장점(Spring 팀에서 권장)
    // 1. final을 통해 불변성을 확보 가능
    // 2. 의존성 보장 가능(순환참조 문제 해결)
    // 3. 코드 가독성 향상(해당 객체가 어떤 의존성을 가지는지 명확히 알 수 있음)
